...
실행 환경: RTX 3090 (23.7GB), Python 3.11, torch 2.7.1+cu118,
transformers 5.16.1

| 항목 | bge-m3 | Qwen3-Emb-4B |
|---|---|---|
| 모델 단독 VRAM | 2.12 GB | 7.54 GB |
| LLM(Qwen3-4B) 동시 로드 | 9.61 GB | 15.04 GB |
| 동시 로드 가능 | O | O |
| 100문장 인코딩 | 0.12초 | 0.41초 |
| 주제 경계 낙차 (한국어) | 0.239 | 0.205 |
| 주제 경계 낙차 (영어) | 0.220 | 0.307 |

낙차 = 같은 주제 내 평균 유사도 - 주제 경계 평균 유사도.
클수록 임계값 설정과 무관하게 경계를 뚜렷하게 잡는다.
테스트 문장은 서로 무관한 3개 주제 × 3문장으로 구성했다.
...
|---|---|---|
| bge-m3 | 0.587 | 0.348 |
| Qwen3-Emb-4B | 0.379 | 0.174 |

Qwen3-Embedding이 전반적으로 낮은 값을 매긴다. 낙차는 비슷하지만
분포 위치가 달라, 같은 고정 임계값이 두 모델에서 정반대로 동작한다.
(0.5 기준: bge-m3는 거의 안 잘리고 Qwen3는 과하게 잘림)


### 선정: bge-m3

1. 품질: 한국어 낙차는 bge-m3가 0.034 높고, 영어는 Qwen3가 0.087 높다.
   한쪽이 일방적으로 우세하지 않으며 본 프로젝트 데이터가 한국어와 영어
   위주라 bge-m3가 불리하지 않다. 다만 테스트 문장이 9개(3주제)로
   표본이 작아 결정적 근거는 아니다.
   
2. 비용: VRAM 3.6배, 인코딩 속도 3.4배 차이가 난다.
   두 모델 모두 LLM과 동시 로드가 가능하지만(9.61 vs 15.04 GB)
   여유 공간이 14GB와 8.7GB로 차이가 크다. 배치 크기를 늘리거나
   긴 문서를 처리할 때 제약이 될 수 있다.
   
3. 어블레이션: 여러 조합을 반복 실행해야 하므로 인코딩 속도가
   실험 횟수를 직접 제한한다.

품질 차이가 뚜렷하지 않은 상황에서 비용 차이가 크므로 bge-m3를
선정한다.
