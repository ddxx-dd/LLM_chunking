...
def _jaccard(u, k):
    inter = max(0.0, min(u.meta["t_end"], k.meta["t_end"]) - max(u.meta["t_start"], k.meta["t_start"]))
...
def align_by_overlap(src_doc, ref_doc):
    ...
    return [
        ref_doc.text[best.start:best.end]
        for u in src_doc.units
        for best in [max(ref_doc.units, key=lambda k: _jaccard(u, k))]
    ]
