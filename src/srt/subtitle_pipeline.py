...
번역, fixed vs semantic vs smart 비교. 큐 단위(Jaccard 시간겹침 정렬) chrF/BLEU/BERTScore 사용.
전체 영화를 돈 뒤 방향별 종합 평균까지 집계한다."""
import sys
...
def default_subtitle_chunkers(embed_model, tokenizer, lang):
    """자막 실험이 쓰는 fixed/semantic/smart 기본 설정(실측 검증된 값).
    lang: 이번 방향의 원문 언어("en"/"ko") - smart의 cue_continues()가 언어별로 다른
    휴리스틱을 쓰므로 방향마다 다시 호출해야 함(호출부인 main()의 방향 루프 참고)."""
    return {
        ...
        "smart": SmartTextSplitter(SmartChunker(
            embed_model, lambda s: len(tokenizer.encode(s, add_special_tokens=False)),
            mode="srt", lang=lang, min_tokens=128, max_tokens=512)),
    }
...
def main():
    ...
    for name, (en_name, ko_name) in SUBTITLE_DATASETS.items():
        ...
        for direction, src_doc, ref_doc in [("ko2en", ko_full, en_full), ("en2ko", en_full, ko_full)]:
            # smart의 cue_continues()가 언어별로 다른 휴리스틱을 써서 방향(원문 언어)마다
            # 다시 만든다 - fixed/semantic은 언어 무관이라 매번 다시 만들어도 결과 동일,
            # 비용도 가벼운 인스턴스 생성뿐이라 무시할 만함.
            chunkers = default_subtitle_chunkers(embed_model, tokenizer, lang=direction[:2])
