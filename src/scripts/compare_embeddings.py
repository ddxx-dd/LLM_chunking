...
def measure_vram(model_path):
    ...
    if TEST_LLM:
        ...
        try:
            ...
            both_ok = True
            print("  [VRAM] 임베딩 + LLM 합계: " + format(total_vram, ".2f") + " GB  -> 동시 로드 성공")
            del llm
...
def measure_gap(model, test_data, label=""):
    ...
    for i in range(len(sentences) - 1):
        ...
        print("    문장{} <-> 문장{}: {:.3f}{}".format(i + 1, i + 2, sim, mark))

    within_avg = float(np.mean(within))
    cross_avg = float(np.mean(cross))
    gap = within_avg - cross_avg

    print("    같은 주제 평균: {:.3f}".format(within_avg))
    print("    주제 경계 평균: {:.3f}".format(cross_avg))
    print("    낙차(gap):      {:.3f}\n".format(gap))
    return round(gap, 3)
...
def main():
    ...
    for r in all_results:
        ...
        print("  LLM 동시 로드:  " + str(r["total_vram"]) + " GB (" + str(r["both_ok"]) + ")")
