...
HTML 뷰어에 넘긴다. 파라미터는 실제 파이프라인 스크립트(run_subtitle_translation.py/
run_docx_compose.py)가 쓰는 값과 동일하게 맞춤 - 뷰어에 나오는 게 실제 실행 결과와
어긋나면 비교 도구로서 의미가 없음."""
import json
import sys
...
from config import SRT_ENG_DIR, DOCX_KOR_DIR, DOCX_ENG_DIR, PDF_KOR_DIR, EMBED_MODEL, RESULTS_DIR
...
from chunking.semantic_chunker import (
    split_sentences, calculate_similarities, calculate_threshold, split_at_boundaries,
)
...
TARGETS = [
    ...
    (DOCX_ENG_DIR / "Gutenberg_PrideAndPrejudice" / "chapter_21.docx", 600,
    ...
    (DOCX_ENG_DIR / "RAG-Multi-Corpus" / "CloudWay-24" / "Baggage Allowance.docx", 600,
     dict(method="percentile", amount=15, min_chunk_tokens=128, max_chunk_tokens=500)),
    (DOCX_KOR_DIR / "Wikipedia_설명문" / "한글.docx", 600,
     dict(method="percentile", amount=15, min_chunk_tokens=128, max_chunk_tokens=500)),
