...
def fragments(doc, chunk):
    ...
    for j, u in enumerate(doc.units):
        s = max(u.start, chunk.start)
        e = min(u.end, chunk.end)
        if s < e:
...
def build_prompt(doc, chunk, instruction="Translate each line to English. Keep the [n] numbers exactly:\n{body}"):
    ...
    body = "\n".join("[{}] {}".format(n + 1, doc.text[s:e]) for n, (j, s, e, w) in enumerate(frs))
...
def chunk_bundle(docs, chunker_fn):
    ...
    docs: Doc 리스트. 반환: [{"doc_idx":i, "doc_name":doc.name, "chunk":Chunk}, ...]"""
    out = []
