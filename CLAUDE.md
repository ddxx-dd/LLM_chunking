...
| 설명문 | Wikipedia_설명문 | docx | 5 | CC BY-SA |
| 설명문/교재 | Wikibooks_WorldHistory(영어) | docx | 31 | CC BY-SA |
| 문학 | Wikisource_문학(한국어 고전) | docx | 16 | CC BY-SA/PD |
| 문학 | Gutenberg(Pride and Prejudice, 챕터 분리) | docx | 61 | Public Domain |
| QA/IR 벤치마크 | trec_covid | docx | 66 | - |
...
지원"으로 단순하게 유지하고 2단인 두 편을 코퍼스에서 뺐다.
...
존재하는 오픈라이선스 코퍼스는 실질적으로 존재하지 않는다.** `RAG-Multi-Corpus`는
저자가 직접 합성 생성한 콘텐츠라 이 문제(개별 문서 저작권 불분명)에서 자유롭고,
전수 확인 결과 320개 표 중 colspan/rowspan 병합이 0건이라 채택했다.
